...
def _log_generation_route(route_type: str) -> None:
    logger.debug("LLM generation route: %s", route_type)


def _generate_with_gemini(query: str, context: str) -> str:
    """Generate an answer using Gemini (text generation model)."""
...
def generate_answer(query: str, context: str, chunks: list[dict[str, object]]) -> str:
    ...
    if os.getenv("GEMINI_API_KEY"):
        try:
            ...
            return _generate_with_gemini(query=query, context=context)
        except Exception as exc:
            logger.warning("Gemini generation failed: %s — falling back", exc)
    ...
    # ── Ollama ────────────────────────────────────────────────────────────────
    try:
        logger.debug("Attempting Ollama generation...")
        _log_generation_route("ollama")
        return _generate_with_ollama(query=query, context=context)
    except Exception as exc:
        logger.warning("Ollama generation failed: %s — falling back to local heuristic", exc)
...
def run_rag_pipeline(
...
) -> tuple[str, list[dict[str, object]], str, list[dict[str, object]] | None]:
    """Run the full RAG pipeline and return (answer, retrieved_chunks, query_type, impact_edges)."""
    if repo_indexed:
        mode = "global"
        file_name = None
    ...
    query_type = classify_query(query)
    logger.info(
        "query=%r intent=%s mode=%s groq_key=%s gemini_key=%s provider=%s",
        query[:80],
        query_type,
        mode,
    ...
    if query_type == "find_usage":
        ...
        if not target:
            return "No target function found in query.", [], query_type
        ...
        if not graph:
            ...
            if re.search(r"\bwhat does\b", normalized_query):
                ...
                if found_callees:
                    return f"The function {target} calls:\n" + "\n".join(list(dict.fromkeys(found_callees))), [], query_type
                return "No usage found in the indexed repository", [], query_type
            ...
            if found_callers:
                return f"The function {target} is used in:\n" + "\n".join(list(dict.fromkeys(found_callers))), [], query_type, None
            return "No usage found in the indexed repository", [], query_type, None

    if query_type == "impact_analysis":
        from app.services.lineage_query import refresh_graph_from_db, expand_with_graph_mode, _matching_nodes
        refresh_graph_from_db()
        
        # Determine if they are asking for downstream impact or upstream source
        mode = "downstream"
        if re.search(r"\b(where|upstream|source\s+of|come\s+from)\b", query, flags=re.IGNORECASE):
            mode = "upstream"
            
        # Try to find the target column or model in the query
        # Just use all words that look like identifiers
        words = set(re.findall(r"\b[a-zA-Z_][a-zA-Z0-9_]*\b", query))
        
        target_nodes = []
        for word in words:
            if word.lower() not in {"what", "how", "where", "is", "the", "of", "from", "in", "on", "if", "drop", "change", "depends", "impact", "break"}:
                matches = _matching_nodes(word)
                if matches:
        ...
        if target_nodes:
            expanded = expand_with_graph_mode(target_nodes, max_depth=3, mode=mode)
            edges = []
            ...
            if edges:
                return f"Found {len(edges)} {mode} dependencies for {', '.join(target_nodes)}.", [], query_type, edges
            else:
                return f"No {mode} dependencies found for {', '.join(target_nodes)}.", [], query_type, None
    ...
    # Always call generate_answer to allow configured LLMs (Gemini, Groq, Ollama)
    ...
    return answer, retrieved_chunks, query_type, None
