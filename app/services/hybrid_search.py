...
# Cross-encoder reranker — loaded lazily so missing sentence-transformers
# doesn't break the server at startup.
...
def hybrid_search(query: str, top_k: int) -> list[dict[str, object]]:
    ...
    if reranker is not None and rerank_candidates:
        try:
            pairs = [
                [query, str(r.get("chunk_text") or "")]
                for r in rerank_candidates
            ]
            scores: list[float] = reranker.predict(pairs).tolist()
            for result, score in zip(rerank_candidates, scores):
            ...
            pass  # fall through to RRF ordering if reranker fails
